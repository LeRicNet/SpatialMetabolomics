...
create_sample_data <- function(condition, sample_id) {
    ...
    # Fix: Ensure we get exactly the number of genes we need
    oxphos_genes <- getOXPHOSGenes("mouse")
    glycolysis_genes <- getGlycolysisGenes("mouse")
    
    # Take up to 50 OXPHOS genes (pad with generic names if needed)
    oxphos_use <- c(oxphos_genes[1:min(50, length(oxphos_genes))],
                    if(length(oxphos_genes) < 50) paste0("OXPHOS_", (length(oxphos_genes)+1):50) else NULL)
    
    # Take up to 20 glycolysis genes (pad with generic names if needed) 
    glycolysis_use <- c(glycolysis_genes[1:min(20, length(glycolysis_genes))],
                        if(length(glycolysis_genes) < 20) paste0("Glycolysis_", (length(glycolysis_genes)+1):20) else NULL)
    
    # Create exactly n_genes gene names
    gene_names <- c(
        oxphos_use[1:50],
        glycolysis_use[1:20], 
    ...
    # Ensure we have exactly n_genes names
    ...
    # Create metadata - fix: ensure row.names length matches
    meta <- data.frame(
        sample_id = rep(sample_id, n_spots_per_sample),
        condition = rep(condition, n_spots_per_sample),
    ...
    # IMPORTANT: Set rownames for coordinates to match colnames of counts
    rownames(coords) <- colnames(base_expr)
...
# Combine data more carefully - FIXED VERSION
...
# CRITICAL FIX: Ensure all rownames/colnames are consistent
cell_names <- colnames(all_counts)

# Set rownames for metadata (should already be correct from create_sample_data)
if(!identical(rownames(all_meta), cell_names)) {
    rownames(all_meta) <- cell_names
}

# Set rownames for coordinates (this is likely the issue)
rownames(all_coords) <- cell_names

# Verify dimensions match before creating SpatialExperiment
stopifnot(
    ncol(all_counts) == nrow(all_meta),
    ncol(all_counts) == nrow(all_coords),
    identical(colnames(all_counts), rownames(all_meta)),
    identical(colnames(all_counts), rownames(all_coords))
)

# Now create SpatialExperiment
...
# First check which genes are actually present
available_oxphos <- intersect(pathways$OXPHOS[1:20], rownames(spm))
available_glycolysis <- intersect(pathways$Glycolysis[1:10], rownames(spm))
test_genes <- c(available_oxphos, available_glycolysis)
...
if (length(test_genes) > 0) {
  ...
  # Fallback: use any available genes from our simulated data
  gene_de <- compareMetabolicStates(
    spm,
    condition = "condition", 
    ref_group = "WT",
    test_group = "SVD",
    features = head(rownames(spm), 30),
    test_type = "wilcox"
  )
}
